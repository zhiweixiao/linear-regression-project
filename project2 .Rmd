...
new_owl_data <- owl_data %>% 
                ...
                select(-c("X1", "id", "date", "stage","t1_odds","t1_probability","t1_place_last_season", 
                          ...
                          "t1_losses_alltime", "t2_matches_alltime", "t2_losses_alltime", "t1_losses_vs_t2", "t1_matches_vs_t2")) %>% 
                na.omit()
...
```{r}
### 10-fold cross validation for three candidate models
library(boot)
set.seed(200)
...
model3_cv_error <- cv.glm(owl_model_data, model2, K = 10)$delta[1]
model3_cv_error
